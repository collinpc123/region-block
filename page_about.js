(function() {
  ...
  function authHeaders() {
    ...
    // ensure accept/content-type present
  ...
  async function fetchAbout(screenName) {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        return { location: "", status: resp.status };
      }
      ...
      return { location: loc || "", status: resp.status };
    } catch (e) {
      console.log("[BTC-page] about fetch exception", e);
      return { location: "", status: 0, error: String(e) };
    }
  }
  ...
  window.addEventListener("message", async (evt) => {
    ...
    const res = await fetchAbout(handle);
    window.postMessage({
